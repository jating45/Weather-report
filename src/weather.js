...
const WeatherWidget = () => {
  ...
  return (
    <div className="weather-container d-flex align-items-center justify-content-center min-vh-100 text-white p-4" style={{ backgroundImage: "url('/background.png')", backgroundSize: "cover" }}>
      <div className="d-flex flex-column flex-md-row align-items-center align-items-md-start gap-4 w-100">
        <div className="text-center text-md-start w-50">
          <h1 className="fw-bold display-6">Real-Time & Historical World Weather Data API</h1>
          <p className="mt-3 text-light fs-5">Retrieve instant, accurate weather information for any location in the world in lightweight JSON format.</p>
          <div className="trusted text-center">
  
  <p className="mt-2 text-light fs-5 fw-bold" >
  <i className="bi bi-trophy-fill"></i> Trusted by 75,000 companies worldwide
  </p>
</div>

          <button className="btn btn-warning mt-3 fw-semibold px-4 py-2 shadow-lg">Start Using the API</button>
        </div>
        
        <div className="weather-widget bg-dark text-white p-4 rounded shadow-lg">
          <h2 className="fs-5 text-uppercase fw-semibold">{CITY}, {weather.condition.text}</h2>
          ...
          <div className="forecast-container mt-4 d-flex justify-content-between">
            {forecast.map((day, index) => (
              <div key={index} className="text-center">
                <p className="fw-semibold">{new Date(day.date).toLocaleDateString("en-US", { weekday: "short" })}</p>
